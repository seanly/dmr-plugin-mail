	...
	resp.Tools = []proto.ToolDef{
		{
			...
			ParametersJSON: `{
				...
				"properties": {
					...
					"htmlBody": {"type": "string", "description": "Optional HTML body (multipart if both text and html)"},
					"attachments": {"type": "array", "items": {"type": "string"}, "description": "Paths resolved under attachment_root / workspace"}
				},
				"required": ["to", "subject"]
			}`,
		},
		{
			...
			ParametersJSON: `{
				"type": "object",
				"properties": {
					"folder": {"type": "string", "description": "IMAP mailbox name, default INBOX"},
					"limit": {"type": "integer", "description": "Max messages (default from config, hard cap 200)"},
					"since": {"type": "string", "description": "RFC3339 or YYYY-MM-DD; internal SINCE"},
					"before": {"type": "string", "description": "RFC3339 or YYYY-MM-DD; internal BEFORE"},
					"unreadOnly": {"type": "boolean", "description": "If true, SEARCH UNSEEN"}
				}
			}`,
		},
		{
			...
			ParametersJSON: `{
				...
				"properties": {
					"folder": {"type": "string"},
					"uid": {"type": "integer", "description": "IMAP UID"},
					"messageId": {"type": "string", "description": "Alternative to uid; exact Message-ID header value"},
					"max_body_chars": {"type": "integer", "description": "Override config max_body_chars"}
				}
			}`,
		},
		{
